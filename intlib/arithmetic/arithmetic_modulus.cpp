...
std::any arithmetic_modulus(const std::any& left_w, const std::any& right_w)
{
	...
	if (detail::is_type<uint64_t>(left_type)) {
		...
		if (detail::is_type<double>(right_type)) {
			...
			if (right < 0) {
				const auto res =
					detail::to_int64(left) % detail::to_int64(right);
				return detail::adapt_type(res);
			}

			const auto res = left % detail::to_uint64(right);
			return detail::adapt_type(res);
		}
	...
	if (detail::is_type<int64_t>(left_type)) {
		...
		if (detail::is_type<double>(right_type)) {
			...
			const auto res = detail::to_uint64(left) % detail::to_uint64(right);
			return detail::adapt_type(res);
		}
	...
	if (detail::is_type<double>(left_type)) {
		...
		if (detail::is_type<uint64_t>(right_type)) {
			...
			const auto res = detail::to_uint64(left) % right;
			return detail::adapt_type(res);
		}
