...
)
{
	...
	if (const auto r =
			any_comparison_greater_than<left_t, int64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than<left_t, uint64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r = any_comparison_greater_than<left_t, double>(left, right);
		r.has_value()) {
		return r;
	}
	return {};
...
)
{
	...
	if (const auto r =
			any_comparison_greater_than_right_numeric<int64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than_right_numeric<uint64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than_right_numeric<double>(left, right);
		r.has_value()) {
		return r;
	}

	if (const auto r =
			any_comparison_greater_than<std::string, std::string>(left, right);
		r.has_value()) {
		return r;
	}
	return {};
