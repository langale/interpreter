...
)
{
	...
	if (const auto r =
			any_comparison_less_equal<left_t, uint64_t>(left_w, right_w);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_less_equal<left_t, std::float64_t>(left_w, right_w);
		r.has_value()) {
		return r;
	}
	return {};
...
any_comparison_less_equal(const WrappedAny& left_w, const WrappedAny& right_w)
{
	...
	if (const auto r =
			any_comparison_less_equal_right_numeric<uint64_t>(left_w, right_w);
		r.has_value()) {
		return r;
	}
	if (const auto r = any_comparison_less_equal_right_numeric<std::float64_t>(
			left_w, right_w
		);
		r.has_value()) {
		return r;
