...
any_comparison_less_equal(const std::any& left, const std::any& right) noexcept
{
...
[[nodiscard]] static std::optional<bool>
any_comparison_less_equal_right_numeric(
...
) noexcept
{
	...
	if (const auto r = any_comparison_less_equal<left_t, int64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r = any_comparison_less_equal<left_t, uint64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r = any_comparison_less_equal<left_t, double>(left, right);
		r.has_value()) {
		return r;
	}
	return {};
...
) noexcept
{
	...
	if (const auto r =
			any_comparison_less_equal<std::string, std::string>(left, right);
		r.has_value()) {
		return r;
	}
	return {};
