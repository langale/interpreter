...
) noexcept
{
	...
	if (const auto r =
			any_comparison_greater_than_equal_to<left_t, bool>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than_equal_to<left_t, int64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than_equal_to<left_t, uint64_t>(left, right);
		r.has_value()) {
		return r;
	}
	if (const auto r =
			any_comparison_greater_than_equal_to<left_t, double>(left, right);
		r.has_value()) {
		return r;
	}
	return {};
...
) noexcept
{
	...
	if (const auto r =
			any_comparison_greater_than_equal_to_right_numeric<uint64_t>(
				left, right
			);
		r.has_value()) {
		return r;
	...
	if (const auto r =
			any_comparison_greater_than_equal_to_right_numeric<double>(
				left, right
			);
		r.has_value()) {
		return r;
